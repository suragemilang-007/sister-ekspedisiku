...
app.post('/kurir/update-status', async (req, res) => {
  try {
    ...
    console.log(`Status update sent to Kafka: ${status} for penugasan ${id_penugasan}`);
    res.status(200).json({ success: true, message: 'Status berhasil dikirim ke Kafka' });
  } catch (error) {
    console.error('Error sending message to Kafka:', error);
    res.status(500).json({ success: false, message: 'Gagal mengirim pesan ke Kafka' });
  }
});
